...
#define WeLog(fmt, ...) //NSLog(fmt, __VA_ARGS__)
...
@implementation DLVolumeButtons
...
{
    [superView addSubview:self.volumeView];
}
...
- (void)audioSessionInterrupted:(NSNotification*)notification {
    ...
    switch (interuptionType) {
        case AVAudioSessionInterruptionTypeBegan:
            WeLog(@"Audio Session Interruption case started.", nil);
            break;
        case AVAudioSessionInterruptionTypeEnded:
        {
            WeLog(@"Audio Session Interruption case ended.", nil);
            NSError *error = nil;
            [self.session setActive:YES error:&error];
            if (error) {
                NSLog(@"%@", error);
        ...
        default:
            WeLog(@"Audio Session Interruption Notification case default.", nil);
            break;
    }
}
...
+ (instancetype)volumeButtonHandlerWithUpBlock:(DLVolumeButtonBlock)upBlock downBlock:(DLVolumeButtonBlock)downBlock {
    DLVolumeButtons *instance = [[DLVolumeButtons alloc] init];
    if (instance) {
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if (context == sessionContext) {
        ...
        WeLog(@"Old Vol:%f New Vol:%f Difference = %f", (double)oldVolume, (double)newVolume, (double) difference);
        
        if (_exactJumpsOnly && difference < .062 && (newVolume == 1. || newVolume == 0)) {
            WeLog(@"Using a non-standard Jump of %f (%f-%f) which is less than the .0625 because a press of the volume button resulted in hitting min or max volume", difference, oldVolume, newVolume);
        } else if (_exactJumpsOnly && (difference > .063 || difference < .062)) {
            WeLog(@"Ignoring non-standard Jump of %f (%f-%f), which is not the .0625 a press of the actually volume button would have resulted in.", difference, oldVolume, newVolume);
            [self setInitialVolume];
            return;
        }
